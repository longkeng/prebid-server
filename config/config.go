...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.Bidder7xbid, "https://match.bnmla.com/usersync/s2s_sync?gdpr={{.GDPR}}&gdpr_consent={{.GDPRConsent}}&r="+url.QueryEscape(externalURL)+"%2Fsetuid%3Fbidder%3Dvisx%26gdpr%3D{{.GDPR}}%26gdpr_consent%3D{{.GDPRConsent}}%26uid%3D%24%7BUUID%7D")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("adapters.7xbid.endpoint", "http://bidder.7xbid.com/api/v1/prebid/banner")
